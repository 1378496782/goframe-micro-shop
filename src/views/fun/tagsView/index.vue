...
export default defineComponent({
	...
	setup() {
		...
		const refreshCurrentTagsView = () => {
			proxy.mittBus.emit('DEMO_WECHAT_OPEN_ID', Object.assign({}, { contextMenuClickId: 0, ...route }));
		};
		// 2、关闭当前 tagsView
		...
		const closeOtherTagsView = () => {
			proxy.mittBus.emit('DEMO_WECHAT_OPEN_ID', Object.assign({}, { contextMenuClickId: 2, ...route }));
		};
		// 4、关闭全部 tagsView
		const closeAllTagsView = () => {
			proxy.mittBus.emit('DEMO_WECHAT_OPEN_ID', Object.assign({}, { contextMenuClickId: 3, ...route }));
		};
		// 5、开启当前页面全屏
		const openCurrenFullscreen = () => {
			proxy.mittBus.emit('DEMO_WECHAT_OPEN_ID', Object.assign({}, { contextMenuClickId: 4, ...route }));
		};
		return {
